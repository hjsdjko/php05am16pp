...
class ApiController extends CommonController
{
    ...
    public function download()
    {
        ...
        header('location:http://' . $_SERVER['HTTP_HOST'] . '/php05am16pp/upload/' . $fileName);
        exit; 
    }
    /**
     * 获取某表的某个字段列表接口
    ...
    public function group($tablesName,$columnName){
        ...
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            ...
            if($tokens['tablename'] == 'yuyuequxiao'){
                if($tokens['tablename'] == 'yisheng'){
                    $where .= " and `yishengzhanghao` = '".$tokens['username']."' ";
                }
            }
            ...
            if($tokens['tablename'] == 'dianzibingli'){
                if($tokens['tablename'] == 'huanzhe'){
                    $where .= " and `huanzhezhanghao` = '".$tokens['username']."' ";
                }
            }
            ...
            if($tokens['tablename'] == 'chufangjilu'){
                if($tokens['tablename'] == 'yisheng'){
                    $where .= " and `yishengzhanghao` = '".$tokens['username']."' ";
                }
            }
    ...
    public function value($tableName,$xColumnName,$yColumnName,$tjType){
        ...
        $where = " where 1 ";
        if ($tokens['isAdmin']!=1 || ($tokens['isAdmin']==1 && $tokens['tablename'] != "users")){
            if($tableName == 'zuozhenyisheng'){
                if($tokens['tablename'] == 'yisheng'){
                    $where .= " and `yishengzhanghao` = '".$tokens['username']."' ";
                }
            }
            if($tableName == 'yuyueguahao'){
                if($tokens['tablename'] == 'yisheng'){
                    $where .= " and `yishengzhanghao` = '".$tokens['username']."' ";
                }
            }
